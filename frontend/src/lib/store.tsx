...
interface AppContextType {
  ...
  sendMessage: (content: string, attachments?: Attachment[], sessionId?: string) => Promise<void>
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // 发送消息
  const sendMessage = useCallback(async (content: string, attachments?: Attachment[], sessionId?: string) => {
    const activeSessionId = sessionId || currentSessionId
    if (!activeSessionId || isStreaming) return

    // 添加用户消息
    const userMsgId = `${activeSessionId}-${Date.now()}-user`
    ...
    // 添加助手消息占位
    const assistantMsgId = `${activeSessionId}-${Date.now()}-assistant`
    ...
    try {
      ...
      await streamChat(fullMessage, activeSessionId, (event) => {
