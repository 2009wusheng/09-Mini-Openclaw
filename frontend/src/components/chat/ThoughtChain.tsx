...
export default function ThoughtChain({ toolCalls }: ThoughtChainProps) {
  ...
  // Auto-expand failed calls
  useEffect(() => {
    displayCalls.forEach((call, idx) => {
    ...
    <div className="mb-2">
      ...
      {isExpanded && (
        <div className="thought-chain space-y-2">
          {toolCalls.length === 0 ? (
            <div className="text-sm text-gray-400 dark:text-gray-500 italic py-2 px-3">
              暂无工具调用
            </div>
          ) : (
            <>
              {displayCalls.map((call, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 工具调用 ID */}
                    {expandedIds[idx] && call.tool_call_id && (
