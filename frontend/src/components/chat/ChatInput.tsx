...
export default function ChatInput() {
  ...
  const { sendMessage, isStreaming, stopStreaming, currentSessionId, multiAgentMode, toggleMultiAgentMode, currentTask, selectSession, newSession } = useApp()
  
  const handleSubmit = async () => {
    const content = input.trim()
    if ((!content && selectedFiles.length === 0) || isStreaming || isUploading) return
    ...
    try {
      ...
      await sendMessage(content || '请帮我查看这些文件', attachments, sessionId || undefined)
      ...
      <div className="flex items-end gap-2">
        <div className="flex-1 relative">
          <textarea
            ...
            rows={1}
            className="w-full resize-none rounded-2xl border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 md:px-6 py-3 md:py-4 pr-24 md:pr-28 text-gray-800 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:border-klein-blue focus:ring-0 focus:outline-none disabled:bg-gray-50 dark:disabled:bg-slate-900 disabled:cursor-not-allowed text-sm md:text-base scrollbar-hide"
          ...
          <div className="absolute right-3 md:right-4 bottom-3 md:bottom-4 flex items-center gap-2">
            <button
              ...
              disabled={isStreaming || isUploading}
              className="p-2 rounded-full text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              title="添加附件"
            >
            ...
            {isStreaming ? (
              <button
                ...
                title="停止生成"
              >
                <Square size={16} className="md:w-4 md:h-4" fill="currentColor" />
              </button>
            ) : (
              <button
                onClick={handleSubmit}
                disabled={isStreaming || isUploading || (!input.trim() && selectedFiles.length === 0)}
                className="p-2 rounded-full bg-klein-blue text-white hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
              >
