...
export default function Sidebar() {
  ...
  const [showMaxSessionsAlert, setShowMaxSessionsAlert] = useState(false)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 max-w-sm mx-4 shadow-xl">
            <div className="text-center">
              ...
              <button
                onClick={() => setShowMaxSessionsAlert(false)}
                className="px-4 py-2 bg-klein-blue text-white rounded-lg hover:opacity-90 transition-opacity"
              >
                知道了
              </button>
            </div>
...
function SkillItem({ name, description, location }: { name: string; description: string; location: string }) {
  ...
  // 转换路径格式：./skills/xxx/SKILL.md -> skills/xxx/SKILL.md
  const filePath = location.replace(/^\.\//, '')
