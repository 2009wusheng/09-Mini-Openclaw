...
async def event_generator(message: str, session_id: str, is_first_message: bool):
    ...
    async for event in agent_manager.astream(message, session_id):
        ...
        # 完成
            ...
            # 保存助手消息 - 合并所有段落到一条消息
            full_assistant_content = "".join(seg["content"] for seg in segments)
            all_tool_calls_combined = []
            for seg in segments:
                if seg.get("tool_calls"):
                    all_tool_calls_combined.extend(seg["tool_calls"])
            agent_manager.session_manager.save_message(
                session_id, "assistant", full_assistant_content, all_tool_calls_combined if all_tool_calls_combined else None
            )
